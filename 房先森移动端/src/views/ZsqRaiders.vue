    ...
    <div class="recommend">
      ...
      <li class="conent" v-for="(item,index) in tuijian" :key="index">
        ...
        <div class="right">
          ...
          <div class="bottom">
            <div>
              <i class="iconfont icon-time"></i>2016-07-07
            </div>
            <div>
              <i class="iconfont icon-view"></i>115
            </div>
          </div>
        </div>
      </li>
...
export default {
  ...
  mounted() {
    this.$axios("/zsqzx.json").then((res) => {
      this.rlist = res.data.data[0].cont;
    });
    this.$axios("/zsqtuijian.json").then((res) => {
